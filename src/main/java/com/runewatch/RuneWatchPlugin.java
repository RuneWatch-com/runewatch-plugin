...
public class RuneWatchPlugin extends Plugin {
	...
	private void showTradeWarning(int groupId) {
		...
		if (!wText.contains("WARNING")) {
			String warningMsg = String.format("<br>WARNING: %s is on the %s list.", trader, rwCase.sourcePossessive());
	...
	private void alertPlayerWarning(String rsn, boolean notifyClear, boolean fc) {
		...
		if (rwCase == null && !notifyClear) {
			...
			response
					.append(String.format(" is on %s list for ", rwCase.sourcePossessive()))
					.append(ChatColorType.HIGHLIGHT)
					.append(rwCase.getReason())
					.append(" ")
					.append(ChatColorType.NORMAL)
					.append("on " + rwCase.date())
					.append(".");
		}
